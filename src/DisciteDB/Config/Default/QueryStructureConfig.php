...
class QueryStructureConfig
{
    ...
    [
        ...
        QueryStructure::Structure->name  => [
            QueryTemplate::Select->name => '{COLUMNS} FROM {DATABASE}.{TABLE} {TABLE}',
            QueryTemplate::SelectAll->name => '{COLUMNS} FROM {DATABASE}.{TABLE} {TABLE}',
        ...
        QueryStructure::Methods->name  => [
            QueryTemplate::Select->name => 'LEFT JOIN {TABLE_FOREIGN} AS {TABLE_ALIAS} ON {TABLE}.{INDEX_KEY} = {TABLE_ALIAS}.{FOREIGN_PRIMARY_KEY}',
            QueryTemplate::SelectAll->name => 'LEFT JOIN {TABLE_FOREIGN} AS {TABLE_ALIAS} ON {TABLE}.{INDEX_KEY} = {TABLE_ALIAS}.{FOREIGN_PRIMARY_KEY}',
